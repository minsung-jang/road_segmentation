...
def soft_skeletonize(x, thresh_width=3):
    ...
    for i in range(thresh_width):
        min_pool_x = -MaxPooling2D((3, 3),strides=(1,1), padding='same')(x*-1)
        down1 = MaxPooling2D((3, 3),strides=(1,1), padding='same')(min_pool_x) - min_pool_x
        contour = Activation('relu')(down1)
        down2 = x - contour
        x = Activation('relu')(down2)
...
def norm_intersection(center_line, vessel):
    ...
    '''
    smooth = 1.
    clf = K.flatten(center_line)
    vlf = K.flatten(vessel)
    intersection = K.sum(clf * vlf)

    return (intersection + smooth) / (K.sum(clf) + smooth)

def soft_cldice_coeff(y_true, y_pred):
    '''
    inputs shape  (batch, channel, height, width).
    ...
    '''
    cl_pred = soft_skeletonize(y_pred)
    target_skeleton = soft_skeletonize(y_true)
    iflat = norm_intersection(cl_pred, y_true)
    tflat = norm_intersection(target_skeleton, y_pred)
    intersection = iflat * tflat
    return  ( (2. * intersection) / (iflat + tflat) )

def soft_cldice_loss(y_true, y_pred):
    loss = 1.0-soft_cldice_coeff(y_true, y_pred)
    return loss
...
def get_unet_128(input_shape=(128, 128, 3),
    ...
    model.compile(optimizer=RMSprop(lr=0.0001), loss=soft_cldice_loss, metrics=[dice_coeff])
...
def get_unet_256_cldice(input_shape=(256, 256, 3),num_classes=1):
    inputs = Input(shape=input_shape)
    # 256
    ...
    model.compile(optimizer=RMSprop(lr=0.0001), loss=soft_cldice_loss, metrics=[soft_cldice_coeff])
